...
export class FormProfilComponent implements OnInit {
  ...
  constructor(private authService: AuthService, private toastr: ToastrService,
              private router: Router, private route: ActivatedRoute, private service: ProfilService) {
  ...
  createForm() {
    this.editForm = new FormGroup({
      ...
      devise: new FormControl('', [Validators.required, Validators.minLength(4)]),
  ...
  get devise() {
    return this.editForm.get('devise');
  }
  ...
  onSubmit() {
    ...
    this.personne.devise = this.devise.value;
